...
export async function getCurrentUserProfile(): Promise<AuthenticatedActorProfileDto> {
  const response = await identityFetch(`${API_GATEWAY_URL}/api/users/me`, {
...
export async function checkPlatformAccess(): Promise<ProtectedAccessDecisionDto> {
  const response = await identityFetch(`${API_GATEWAY_URL}/api/permissions/authenticated-platform-access`, {
    headers: await getGatewayHeaders(),
  })
