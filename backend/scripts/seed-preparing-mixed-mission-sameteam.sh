...
#   * Two participants on the SAME team, associated to the session:
#       - samuelpl888@gmail.com / samuel1234  }  both on team "Demo Team" (code DEMOTEAM)
#       - grisel@gmail.com      / grisel1234  }
#     Both are created in Keycloak with emailVerified=true and the Participant role.
#
# Sibling of seed-preparing-mixed-mission.sh (which puts each participant on their OWN team).
# This variant uses its own mission/quiz/session/team names so the two can coexist.
#
...
MISSION_NAME="Preparing Demo Mixed Mission (Shared Team)"
QUIZ5_TITLE="Demo Trivia 5 Preguntas (Shared)"
QUIZ2_TITLE="Demo Trivia 2 Preguntas (Shared)"
SESSION_TITLE="Preparing Demo Mixed Shared"
...
SUB1_CLUE_ID="$(psql_md_val "
  ...
  VALUES ($SUB1_ID, 'Pista inicial', 1, 'Busca en plaza mickey la placa.', 'VisibleWhenSubstageStarts')
  RETURNING \"Id\";")"
...
psql_md -c "
  ...
  VALUES
    ($SUB1_ID, 'Universidad Católica Andrés Bello', 'SHARED-A-1', 1, true, 10.46426, -66.97629, 50, $SUB1_CLUE_ID),
    ($SUB1_ID, 'Plaza Altamira', 'SHARED-A-2', 2, true, 10.49559, -66.84886, 50, $SUB1_REL_CLUE_ID),
    ($SUB1_ID, 'Plaza Bolívar de Caracas', 'SHARED-A-3', 3, true, 10.50661, -66.91461, 50, NULL);
"
...
psql_md -c "
  INSERT INTO \"MissionTargets\" (\"SubstageId\", \"Name\", \"QrCode\", \"SequenceOrder\", \"IsActive\", \"Latitude\", \"Longitude\", \"Score\", \"ClueId\")
  VALUES ($SUB4_ID, 'Parque del Este', 'SHARED-B-1', 1, true, 10.49820, -66.83630, 50, $SUB4_CLUE_ID);
"
...
echo "  team (both participants on ONE team) …"
TEAM_DISPLAY="Demo Team"
TEAM_CODE="DEMOTEAM"
TEAM_MEMBERS="samuelpl888 grisel"

SHARED_TEAM_ID="$(app_register_team "$APP_ADMIN_TOKEN" "$TEAM_DISPLAY" "$TEAM_CODE" || true)"
[[ -z "$SHARED_TEAM_ID" ]] && { echo "  FAILED: could not register/resolve team '$TEAM_CODE'" >&2; exit 1; }
echo "    team ready: $TEAM_DISPLAY ($TEAM_CODE) → $SHARED_TEAM_ID"

for tmember in $TEAM_MEMBERS; do
  uid="${APP_USER_ID[$tmember]:-}"
  [[ -z "$uid" ]] && { echo "    SKIP $tmember (no app user id)"; continue; }
  status="$(app_assign_participant "$APP_ADMIN_TOKEN" "$SHARED_TEAM_ID" "$uid")"
  case "$status" in
...
declare -a TEAM_IDS=("$SHARED_TEAM_ID")
...
echo "  Team        : $TEAM_DISPLAY ($TEAM_CODE) — both participants"
echo "  Participants: samuelpl888@gmail.com / samuel1234  }  → $TEAM_DISPLAY"
echo "                grisel@gmail.com      / grisel1234  }"
