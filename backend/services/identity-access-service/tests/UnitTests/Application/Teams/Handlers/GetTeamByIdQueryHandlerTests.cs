...
public sealed class GetTeamByIdQueryHandlerTests
{
    ...
    public async Task Handle_ReturnsTeamForAuthorizedActor(Role actorRole)
    {
        ...
        var result = await handler.Handle(new GetTeamByIdQuery(team.TeamId), CancellationToken.None);

        result.TeamId.Should().Be(team.TeamId);
        result.DisplayName.Should().Be("Red Team");
        result.TeamCode.Should().Be("RED-01");
    }
    ...
    public async Task Handle_ThrowsWhenTeamDoesNotExist()
    {
        ...
        var act = async () => await handler.Handle(new GetTeamByIdQuery(Guid.NewGuid()), CancellationToken.None);
    ...
    private static GetTeamByIdQueryHandler CreateHandler(
