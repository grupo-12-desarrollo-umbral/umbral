...
{
    [Fact]
    public async Task Handle_ReenablesKeycloakAccountThenPersistsUpdate()
    {
        ...
        await handler.Handle(new ReactivateUserCommand(target.Id), CancellationToken.None);

        identityProvider.Verify(
            svc => svc.SyncUserActiveStateAsync(target.ExternalIdentityId, true, It.IsAny<CancellationToken>()),
            Times.Once);
        updatedUser.Should().NotBeNull();
        updatedUser!.IsActive.Should().BeTrue();
        updatedUser.DomainEvents.Should().ContainSingle(eventItem => eventItem is UserAccessReactivatedEvent);
        repository.Verify(repo => repo.UpdateAsync(target, It.IsAny<CancellationToken>()), Times.Once);
    }

    [Fact]
    public async Task Handle_KeycloakSyncFails_DoesNotPersistReactivation()
    {
        var actor = User.Provision("kc-admin", "Admin", "admin@example.com", Role.Administrator);
        actor.Id = 1;
        ...
        var identityProvider = new Mock<IIdentityProviderAdminService>();
        identityProvider
            .Setup(svc => svc.SyncUserActiveStateAsync(It.IsAny<string>(), true, It.IsAny<CancellationToken>()))
            .ThrowsAsync(new IdentityProviderUserStateSyncException(
                target.ExternalIdentityId, true, "Keycloak Admin API unavailable after retries."));

        var handler = new ReactivateUserCommandHandler(
            repository.Object, currentActor.Object, new AccessPolicy(), identityProvider.Object);

        var act = async () => await handler.Handle(new ReactivateUserCommand(target.Id), CancellationToken.None);

        // Keycloak-first: the sync failure surfaces (503) and the app DB is never committed, so
        // both stores stay disabled — no silent half-open state.
    ...
    public async Task Handle_RejectsDeactivatedActor()
    {
        ...
        var act = async () => await handler.Handle(new ReactivateUserCommand(42), CancellationToken.None);
