...
public sealed class TeamRepository : ITeamRepository
{
    ...
    public async Task AddAsync(RegisteredTeam team, CancellationToken cancellationToken)
    {
        ...
        await SaveTranslatingTeamCodeConflictAsync(team, cancellationToken);
    }
    ...
    // The handlers check TeamCodeExistsAsync before writing, but that check and this write are not
    // atomic: two concurrent registrations for the same code both pass the check, then the loser's
    // INSERT/UPDATE collides on the registered_teams.team_code unique index. Translating the raw
    // 23505 into the domain-owned conflict keeps the Npgsql exception from escaping into Application
    // and surfaces the same 409 the pre-check produces, rather than a 500.
    private async Task SaveTranslatingTeamCodeConflictAsync(RegisteredTeam team, CancellationToken cancellationToken)
    {
        try
        {
        ...
        {
            throw new TeamCodeAlreadyExistsException(team.TeamCode);
        }
    }
}
