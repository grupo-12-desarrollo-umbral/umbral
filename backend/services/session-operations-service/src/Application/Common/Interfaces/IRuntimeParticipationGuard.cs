...
// re-checks the users-service eligibility fact and, on deny, applies a Participation Block before rejecting.
// Returns the access fact it validated so callers that also need the eligible-teams whitelist (reconnect's
// first-join branch) can reuse it instead of making a second users-service round-trip.
