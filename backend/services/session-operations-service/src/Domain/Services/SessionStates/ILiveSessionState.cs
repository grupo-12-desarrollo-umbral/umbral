...
internal interface ILiveSessionState
{
    ...
    AuthoritativeSessionTimerSnapshot GetMissionTimerSnapshot(LiveSession session, DateTimeOffset observedAt);

    AuthoritativeSessionTimerSnapshot MarkMissionTimerExpiredIfElapsed(LiveSession session, DateTimeOffset occurredAt);

    bool IsMissionTimerAdvancing(LiveSession session);
