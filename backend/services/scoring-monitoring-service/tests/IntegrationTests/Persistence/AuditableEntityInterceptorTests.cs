...
// Modified (advances LastModified only), the unchanged-with-no-owned-changes skip, and the
// null-context guard on both the sync and async entry points. An in-memory model keeps it
// broker/DB-free; a mutable FixedTimeProvider gives deterministic timestamps.
public sealed class AuditableEntityInterceptorTests
{
    ...
    public async Task SavingChangesAsync_AddedEntity_StampsCreatedAndModified()
    {
        ...
        var interceptor = new AuditableEntityInterceptor(User("creator").Object, new FixedTimeProvider(created));
        ...
        thing.CreatedBy.Should().Be("creator");
        thing.LastModified.Should().Be(created);
        thing.LastModifiedBy.Should().Be("creator");
    }
    ...
    public void SavingChanges_ModifiedEntity_AdvancesLastModifiedOnly()
    {
        ...
        var interceptor = new AuditableEntityInterceptor(User("editor").Object, clock);
    ...
    {
        var interceptor = new AuditableEntityInterceptor(User("nobody").Object, new FixedTimeProvider(DateTimeOffset.UnixEpoch));
