...
// Covers ConcurrencyRetryBehaviour: a handler that lost an optimistic-concurrency race on the ranking
// projection is re-run against fresh state, and tracking is reset first so the retry re-reads instead
// of recomputing on the stale ranking it already lost with.
public sealed class ConcurrencyRetryBehaviourTests
{
    private sealed record Request;

    private static ConcurrentRankingModificationException Loss() =>
    ...
    public async Task Handle_WhenFirstAttemptLosesRace_RetriesAndReturnsSecondResult()
    {
        ...
        unitOfWork.Verify(
            ...
            Times.Once,
            "the losing attempt's stale ranking must be dropped before the handler re-reads");
    }
    ...
    public async Task Handle_WhenEveryAttemptLosesRace_ThrowsAfterExhaustingBudget()
    {
        ...
        await act.Should().ThrowAsync<ConcurrentRankingModificationException>(
    ...
    public async Task Handle_WhenHandlerThrowsOtherException_DoesNotRetry()
    {
        ...
        var act = async () => await behaviour.Handle(
            ...
            {
                attempts++;
                throw new ScoreEntryIsAppendOnlyException();
            },
            CancellationToken.None);

        await act.Should().ThrowAsync<ScoreEntryIsAppendOnlyException>();
