...
public static class ObservabilityExtensions
{
    ...
    /// </summary>
    /// <remarks>
    /// The guard is not an optimisation. <c>MissionDesignApiWebApplicationFactory</c> boots the
    ...
    public static void AddObservability(this IHostApplicationBuilder builder)
    {
        ...
        // Endpoint, protocol and service name all come from the OTEL_* environment, so nothing here
        // depends on this service's appsettings.json. Seq speaks OTLP over HTTP only: the exporter's
        // default gRPC transport fails against it with HTTP_1_1_REQUIRED, and fails silently, so
        // OTEL_EXPORTER_OTLP_PROTOCOL=http/protobuf is required alongside the endpoint.
        builder.Services.AddOpenTelemetry()
            .WithTracing(tracing => tracing
                ...
                // No outbound HTTP call sites today, but the instrumentation costs nothing until one
                // exists and then records its span with no call-site change.
