...
public sealed class MissionInfrastructureIntegrationTests : IClassFixture<PostgreSqlFixture>
{
    ...
    public async Task MissionRepositoryAndReadModelRepository_RoundTripMissionComposite()
    {
        ...
        var target = mission.AddTarget(stage.Id, treasureSubstage.Id, "Target 1", "QR-001", 1, 4.711, -74.0721);
        ...
        reloadedMission.Stages[0].Substages.ElementAt(0).Targets[0].Coordinates.Latitude.Should().Be(4.711);
        reloadedMission.Stages[0].Substages.ElementAt(0).Targets[0].Coordinates.Longitude.Should().Be(-74.0721);
