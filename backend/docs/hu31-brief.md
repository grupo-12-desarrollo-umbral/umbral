...
| X.4 Api | Participant `target-scans` endpoint returns acceptance/outcome (no score leak) on a correct scan; wrong/duplicate/out-of-context → consistent RFC 7807; non-admitting session rejected; E2E proves `EvidenceSubmissionRegistered` then `TargetResolved` end-to-end; ADR-0005 coverage (aggregate line+branch ≥ 93) | — |
