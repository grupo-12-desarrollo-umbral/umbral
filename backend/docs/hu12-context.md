...
| Source readiness for mission substages | Ensure only published quizzes are selectable by trivia substages; draft or archived quizzes are not selectable |
| Historical retention | Archive a quiz to withdraw it from future substage selections without destructive removal |
...
- **Pattern placement validated (ADR-0012) — Domain-only is intentional; do not re-flag.** The mandated `Template Method` lives in the genuine domain template `TriviaQuizLifecycleTemplate` (fixed publish/archive skeleton + overridable steps + ≥2 concretes). There is deliberately **no** app-layer `Trivias/Common/Lifecycle/` base validator: the lifecycle command validators are one `NotEmpty` rule each, so a shared base would be the exact ceremony ADR-0012 warns against (a "skeleton" of one rule + a hook nobody overrides = plain inheritance, collapse it). Resolved no-op — see `backend/plans/pattern-placement-remediation.md` §4. Add an app base only if real shared rules emerge.
