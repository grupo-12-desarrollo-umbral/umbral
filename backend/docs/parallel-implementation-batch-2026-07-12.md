...
and DES-40/DES-41 were resolved as Duplicate of DES-95. On **2026-07-14** three same-story pairs were
merged — DES-51 (HU-37) + DES-54 (HU-39) → **DES-99** (score ledger + ranking), DES-56 (HU-40A) +
DES-57 (HU-40B) → **DES-100** (session history), and DES-48 (HU-35) + DES-50 (HU-36B) → **DES-101**
(trivia results on question close), originals canceled — so the active Todo set is now
**11** (was 14). None of these are in the startable batch below; the merges only reshape the
downstream sequencing (see "Sequencing after the batch").
...
Next wave (after the batch lands): **DES-13 + DES-95 + DES-99** (DES-99 = merged HU-37 ledger +
HU-39 ranking), then DES-43.
...
|---|---|---|---|
| DES-29 | HU-21 session-state audit | 2026-07-13 | Unblocked DES-100 (HU-40, formerly DES-56/HU-40A) |
...
```
DES-42 ──▶ DES-99 (Lane B ledger + ranking, merged) ──▶ DES-35/101/33 and DES-100   (DES-34/HU-25A canceled 2026-07-13, absorbed into DES-96)
DES-42 + DES-95 ──▶ DES-43
DES-42 ──▶ DES-53 ──▶ DES-100 (session history, merged; also blocked by DES-99)
```

DES-99 folds the former DES-51 (HU-37 ledger) → DES-54 (HU-39 ranking) chain into one ticket,
DES-100 folds DES-56 (HU-40A) → DES-57 (HU-40B) into one, and DES-101 folds DES-48 (HU-35) → DES-50
(HU-36B) trivia-close reveal into one. After the batch, the next wave is
DES-13 + DES-95 + DES-99. DES-43 becomes startable after both DES-42 and DES-95 land.
...
- Downstream note (not in this batch): **DES-100** (HU-40 history, merged from DES-56/DES-57) is
  `blockedBy` **DES-92**, an enabler currently in **Backlog**. Its prerequisite **GH #164**
  (MassTransit bootstrap) closed 2026-07-12, so DES-92 is no longer GH-gated (its Linear
  `blockedBy` is now empty) — it simply hasn't been pulled from Backlog. The history layer still
  can't complete on the active Todo tickets alone, because DES-92 sits outside that set.
