...
- **Publisher code is correct and well-built**, not something to "fix". `session-operations-service` publishes three integration events (`AnswerRegistered`, `QuestionClosed`, `SessionResultsFinalized`) via a hand-rolled `RabbitMqIntegrationEventPublisher` (RabbitMQ.Client 7.1.2) behind `IIntegrationEventPublisher`. Publishing is correctly decoupled from the DB transaction (post-commit dispatch via `DispatchDomainEventsInterceptor` in `SavedChanges`, plus a non-blocking in-memory bounded channel hand-off). Best-effort by design (D-3): messages can be dropped on overflow/broker failure, no durable outbox. Not a bug — intentional.
  - **> Amendment (2026-07-12, post-#164 MassTransit migration):** the "non-blocking / best-effort by design (D-3) ↔ not a bug" characterisation above is accurate **only for events still on the hand-rolled `RabbitMqIntegrationEventPublisher`** (currently `AnswerRegistered`, `SessionResultsFinalized`; pending `#165` migration). `QuestionClosed` moved onto MassTransit via #164 (`IPublishEndpoint.Publish` dispatched synchronously post-commit), which is **bounded-*blocking*, not structurally non-blocking** — MassTransit retry-blocks on a down broker; the only thing preventing an indefinite commit-thread stall is the 5s linked-timeout token the handler must carry on every call. "Never faults the runtime" survives; **"zero added commit latency" does NOT** — a broker outage now stalls the commit for ≤5s per publish, not 0s. Guardrail is **per-call, not structural**: forget the timeout on any one publish and the indefinite hang returns. Each future MassTransit migrate (`#165` for `SessionResultsFinalized` + `AnswerRegistered`; DES-92 `ClueReleased`) MUST carry the same `CreateLinkedTokenSource(ct) + CancelAfter(5s)` wrapping plus `catch (OperationCanceledException)`. Full analysis: [`gh-164-masstransit-integration-test-hang-postmortem-2026-07-12.md`](./gh-164-masstransit-integration-test-hang-postmortem-2026-07-12.md) § "D-3 is downgraded…", forward guardrails §1, §2.
