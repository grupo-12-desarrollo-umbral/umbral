...
**SignalR** `SessionStateChanged` broadcast. This slice is the **audit-write half** that was split out
of the old `HU-21B`: it (1) records each valid transition as an **append-only `SessionEvent`** capturing
date + responsible user + reason, and (2) after transactional success **publishes `SessionStateChanged`
to RabbitMQ** for async audit — without the transition depending on the broker. It adds **no** new
endpoint, **no** new state/edge/validator, and **no** frontend surface. The durable queryable history
(`AuditHistory`) is DES-56/HU-40A's, not this slice's.
...
- after transactional success, `SessionStateChanged` is **published to RabbitMQ** (routing key `session.state.changed`), **off the critical path** (broker failure swallowed, transition still commits)
...
- **no** MassTransit — mirror the existing `IIntegrationEventPublisher` / raw `RabbitMQ.Client` publisher
...
- `Facade` (mandated, X.2) — ADR-0004 "session orchestration **+ outbound event publication**"; matrix "**Facade publishes the event**." Realized by the transition handler + a **post-commit** `PublishSessionStateChangedIntegrationEventHandler` (no standalone `*Facade.cs`, per ADR-0012).
- Transport `RabbitMQ` (X.2/X.3/X.4) — `SessionStateChanged` published for async audit after transactional success, never on the critical path. **SignalR** is HU-21A's and untouched.

> Gate obligation: `State` and `Chain of Responsibility` appear as **verify** gates in X.1/X.2; the
> `Facade`/RabbitMQ event-publication boundary is a **build** gate in X.2 (publish handler that swallows
> broker failure) proven end-to-end in X.4.
...
(`int?`). The RabbitMQ publisher backbone is already in the service: `IIntegrationEventPublisher` +
`RabbitMqIntegrationEventPublisher` (raw `RabbitMQ.Client`, durable topic exchange, best-effort outbox) + the
`Publish<X>IntegrationEventHandler` pattern + flat integration-event records in `Application/Sessions/Common/`.
No same-service predecessor is In Progress, so the branch base is `develop`.
...
| Enriched domain event | `SessionStateChangedEvent` gains `ResponsibleUserId` + `Reason` (+ actor type). |
| Responsible-user threading | Handler surfaces the operator numeric `UserId` (via `IAuthenticatedActorProfileAccessClient`, already fetched in the auth resolver) → `SessionTransitionContext` → `MoveTo` → event + `SessionEvent`. |
| RabbitMQ audit publish | `SessionStateChangedIntegrationEvent` + `PublishSessionStateChangedIntegrationEventHandler` (mirror the existing publish handlers) + routing key `session.state.changed`; published after commit, broker failure swallowed. |
| Persistence | New `session_events` table (EF config + migration); `live_sessions` unchanged. |
...
> README, or Linear state may have changed since 2026-07-12.

```text
Read and summarise:
- @backend/services/session-operations-service/CONTEXT.md — §SessionState, §SessionEvent (:91-93), §Facade (:209-211)
...
Output: what HU-21A already shipped (do not rebuild), what HU-21 adds (SessionEvent record + RabbitMQ
SessionStateChanged publish), and confirmation that no queryable audit-history surface is in scope.
...
- the reason is preserved when supplied
- after transactional success, SessionStateChanged is published to RabbitMQ, off the critical path
...
- SessionStateChangedEvent now carries ResponsibleUserId + Reason
...
- PublishSessionStateChangedIntegrationEventHandler maps SessionStateChangedEvent → SessionStateChangedIntegrationEvent
  and publishes via IIntegrationEventPublisher; a broker/publish failure is swallowed (act.Should().NotThrowAsync())
...
- Gate: Chain of Responsibility verified — ordered composable validators; Facade/event-publication boundary present
  as a post-commit notification handler (no standalone *Facade.cs)
...
- ef migrations add produces a migration adding session_events (append-only child of live_sessions) with
  occurred_at/actor_type/actor_id/payload_summary/correlation_id, and no change to live_sessions
- a persistence integration test round-trips the SessionEvent appended by a transition (date + actor + reason)
- the session.state.changed routing key const + ResolveRoutingKey switch arm are wired in RabbitMqIntegrationEventPublisher
- Gate: RabbitMQ transport is the existing durable topic exchange — no MassTransit, no bespoke outbox
...
- end-to-end integration test: a valid transition → 200, a SessionEvent persisted (date + responsible user + reason),
  and SessionStateChanged published to the broker on routing key session.state.changed
- the transition still commits when the broker is unavailable (RabbitMQ off the critical path, AC #4)
...
- confirm a SessionEvent row was appended (persistence/log) and a message reached the RabbitMQ topic
  exchange umbral.session-operations with routing key session.state.changed
- stop the rabbitmq container and repeat the transition → it must still return 200 (broker off the critical path)

Output: container status, smoke results, and confirmation the transition commits with the broker down.
```
...
published as `SessionStateChanged` to RabbitMQ for asynchronous audit/history — mirroring the service's
existing `IIntegrationEventPublisher` backbone and its post-commit `DispatchDomainEventsInterceptor`, so the
transition never depends on broker availability. The queryable consolidation of that history is intentionally
**out of scope** here: it is owned by DES-56 (HU-40A) and scoring-monitoring (DES-85), which consume this
event. This is why DES-29's original query AC was stripped and why DES-29 `blocks` DES-56.
