...
- **Pattern placement validated (ADR-0012) — Domain-only is intentional; do not re-flag.** The mandated `Template Method` lives in the genuine domain template `TriviaQuizReuseWorkflowTemplate` (fixed duplicate/retire/remove skeleton + overridable steps + ≥2 concretes). There is deliberately **no** app-layer `Trivias/Common/Reuse/` base validator: the reuse command validators are one `NotEmpty` rule each, so a shared base would be the exact ceremony ADR-0012 warns against (a "skeleton" of one rule + a hook nobody overrides = plain inheritance, collapse it). Resolved no-op — see `backend/plans/pattern-placement-remediation.md` §4. Add an app base only if real shared rules emerge.
