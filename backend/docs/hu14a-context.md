...
- **Pattern matrix is unaffected.** The required-pattern obligation for HU-14A is `Template Method` over add/update question validation (`required_patterns_matrix.md`), which is satisfied. Removing questions is not pattern-mandated and does not restore any handler base-class requirement.
