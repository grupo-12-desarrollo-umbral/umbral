...
| X.4 Api | New `MultiDeviceTeamSyncHubTests` asserting AC1–AC4 together (two devices both receive `TeamBoardUpdated`; participant present until last connection drops; other-team device never receives the board; reconnect admits idempotently + current state/timer) + ADR-0005 aggregate branch coverage of at least 95%. No new endpoint | — |
