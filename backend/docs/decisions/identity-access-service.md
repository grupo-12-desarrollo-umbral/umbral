...
## [017] HU-05 Phase Y.1 — Domain layer for participant-to-team assignment
**Date:** 2026-05-31
**Phase:** Y.1 — Domain Layer (HU-05)
**Commits:** 6f36ea2
**HU tickets advanced:** HU-05, DES-9, DES-67

**What was built**
`TeamMembership` value entity linking a participant (`userId`) to a `Team`, with invariants: active team required, no duplicate assignments. `ParticipantAssignedToTeamEvent` domain event. Typed exceptions: `ParticipantAlreadyAssignedToTeamException`, `TeamNotActiveException`. Unit tests covering assignment rejection on inactive team — 5 files, 85 lines.

**Why this approach**
`TeamMembership` is a separate entity (not a collection on `Team`) because membership has its own lifecycle and domain events. The `TeamNotActiveException` guard lives in the `TeamMembership` constructor (not the `Team` aggregate) to keep the invariant near the data it protects. The `Team` entity was extended with an `AddMembership` method that validates `IsActive` before delegating to the `TeamMembership` constructor.

**Deliberately skipped**
- Application layer (command, handler, validator) — deferred to phase Y.2
- Persistence / EF Core configuration — deferred to phase Y.3
- API endpoints — deferred to phase Y.4

**Next session needs to know**
Phase Y.1 builds clean. Proceed to Phase Y.2: `AssignParticipantToTeamCommand` with handler enforcing participant role and team active state.

---

## [018] HU-05 Phase Y.2 — Application layer for participant assignment
**Date:** 2026-05-31
**Phase:** Y.2 — Application Layer (HU-05)
**Commits:** 273fb73
**HU tickets advanced:** HU-05, DES-9, DES-67

**What was built**
Application-layer command, handler, and validator for participant assignment: `AssignParticipantToTeamCommand` with `[Authorize(Roles = "Administrator,Operator")]`, validator (checks team exists and is active, user exists and has Participant role, no duplicate membership). `AssignParticipantToTeamCommandHandler` (validates actor authorization via `AccessPolicy`, delegates to `team.AddMembership()`, persists via `ITeamRepository.UpdateAsync`). `GetTeamParticipantsQuery` + handler for listing. `UserNotParticipantRoleException`. Extended `Team` entity with `AddMembership` and `Memberships` navigation. Unit tests cover: successful assignment with event emission, duplicate rejection, inactive-team rejection, non-participant rejection, non-admin caller rejection — 18 files, 618 inserts.

**Why this approach**
Two-step authorization (attribute + `AccessPolicy.Evaluate`) follows the established pattern from HU-03. The validator performs all existence checks so the handler can assume valid inputs. The `GetTeamParticipantsQuery` reuses the existing `ITeamRepository.GetByIdAsync` with `Include(m => m.Memberships)` eager loading.

**Deliberately skipped**
- Infrastructure/persistence layer for `TeamMembership` — deferred to phase Y.3
- API endpoint wiring — deferred to phase Y.4
- Participant-removal or transfer — not in HU-05 scope

**Next session needs to know**
Phase Y.2 builds clean; all unit tests pass. Proceed to Phase Y.3: add `TeamMembershipConfiguration` to EF Core, generate migration, and write integration tests.

---

## [019] HU-05 Phase Y.3 — Infrastructure persistence for team memberships
**Date:** 2026-05-31
**Phase:** Y.3 — Infrastructure Layer (HU-05)
**Commits:** bc99194
**HU tickets advanced:** HU-05, DES-9, DES-67

**What was built**
EF Core `TeamMembershipConfiguration`, updated `TeamConfiguration` with `HasMany(m => m.Memberships)`, updated `ApplicationDbContext` with `DbSet<TeamMembership>`, and EF migration `AddTeamMemberships` creating the `TeamMemberships` table with composite unique index on `(TeamId, UserId)`. Extended `TeamRepository` to include `Memberships` on `GetByIdAsync`. Integration tests covering: successful participant assignment round-trip, duplicate assignment rejection, participant listing by team, and inactive-team assignment rejection — 11 files, 575 inserts.

**Why this approach**
The composite unique index on `(TeamId, UserId)` enforces the no-duplicate-assignment invariant at the database level, complementing the application-layer check. The `TeamMembershipConfiguration` uses the established EF Core fluent API pattern. Integration tests use the existing Testcontainers PostgreSQL fixture.

**Deliberately skipped**
- API layer wiring — deferred to phase Y.4
- Participant-removal endpoints — not in HU-05 scope

**Next session needs to know**
Phase Y.3 passes its gate: build succeeds, integration tests green. Proceed to Phase Y.4: wire `POST /api/teams/{id}/participants` and `GET /api/teams/{id}/participants` in the API layer.

---

## [020] HU-05 Phase Y.4 — API layer for participant assignment endpoints
**Date:** 2026-05-31
**Phase:** Y.4 — API Layer (HU-05)
**Commits:** 621ea4f, e87337e
**HU tickets advanced:** HU-05, DES-9, DES-67

**What was built**
Two new API endpoints on `TeamsEndpoints`: `POST /api/teams/{teamId:int}/participants` (assign participant, returns 201 with location header) and `GET /api/teams/{teamId:int}/participants` (list participants for a team, operator+ role). Extended `ProblemDetailsExceptionHandler` for participant-specific exceptions. Integration tests covering: successful assignment, duplicate assignment 409, inactive-team 422, non-participant-user 400, non-admin 403, `GET` returns assigned participants with correct DTO shape. Frontend `TeamsPanel.tsx` and `teams.ts` actions wired to the new endpoints.

**Why this approach**
Endpoints delegate to the existing application-layer command/query from phase Y.2. `POST` returns 201 with `Location: /api/teams/{teamId}/participants` to follow REST resource-creation conventions. Duplicate assignment returns 409 Conflict (matching the HU-04 duplicate-code convention). The frontend integration was done in the same phase because the participant assignment UI depends on the backend contract being stable.

**Deliberately skipped**
- Participant removal (`DELETE /api/teams/{teamId}/participants/{userId}`) — identified as a potential DES ticket but not in HU-05 scope
- OpenAPI metadata — deferred to a documentation pass
- Frontend error-handling polish — basic toast feedback is wired; edge cases can be improved incrementally
