...
- **Pattern placement validated (ADR-0012) — no local `*AuthorizationProxy`, intentional; do not re-flag.** The mandated `Proxy` access decision is owned **upstream** by identity-access's genuine `ParticipantMembershipAccessAuthorizationProxy` (HU-06, a PASS proxy), called here via `IParticipantMembershipAccessClient`. Locally, `JoinPolicy.EnsureCanReconnect` is **domain admission** (session state, role+team assignment), not an access proxy. A local proxy around `ReconnectAuthenticatedParticipantCommandHandler` would only move the existing cross-service call one hop (pass-through, no new guarantee) — ADR-0012 routes a guard with no local resource decision to `AuthorizationBehaviour`/upstream. Resolved no-op — see `backend/plans/pattern-placement-remediation.md` §5.
