...
- published contract for the HU-37A/history consumer: exchange `umbral.session-operations` (durable topic); routing keys `session.question.closed` → `QuestionClosedIntegrationEvent`, `session.results.finalized` → `SessionResultsFinalizedIntegrationEvent`
  > ⚠️ **STALE TOPOLOGY (2026-07-12).** This documents the *original raw `RabbitMQ.Client`* contract. The MassTransit migration **GH #164 → #165 → #166** replaces it: #164 migrates `QuestionClosed`, #165 migrates `SessionResultsFinalized` + `AnswerRegistered`, #166 deletes `RabbitMqIntegrationEventPublisher` and drops the single `umbral.session-operations` topic exchange. After the chain lands, consumers bind to **MassTransit exchange-per-type** (`[EntityName("session-question-closed")]`, `session-results-finalized`) via `IConsumer<...>` — **not** the durable-topic exchange + `session.*` routing keys above. DES-45/HU-33B stays Done and behaviour-preserving; only this documented binding changes.
