...
- **Pattern placement validated (ADR-0012) — no local `*AuthorizationProxy`, intentional; do not re-flag.** The mandated `Proxy` access decision is owned **upstream** by identity-access's genuine `ParticipantMembershipAccessAuthorizationProxy` (HU-06, a PASS proxy). Locally, `JoinPolicy.EnsureCanJoin/EnsureCanReconnect` is **domain admission** (session state, capacity, role+team assignment), not an access proxy. A local `session-operations` proxy would just relocate the existing cross-service `IParticipantMembershipAccessClient` call (pass-through, no new guarantee) — and ADR-0012 routes a guard with no local resource decision to `AuthorizationBehaviour`/upstream, not a `*AuthorizationProxy`. Resolved no-op — see `backend/plans/pattern-placement-remediation.md` §5.
