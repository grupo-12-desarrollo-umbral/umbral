...
user's role. Operators see the same list **read-only**.
...
### Step E — Operator sees the list read-only

1. Sign out, sign in as **op-1 / operator123**, open **Users**.
2. The list renders, but there is **no invite form** and **no Deactivate/Change
   role** buttons. Operators can view users, not manage them.
...
| Server-side guard    | As op-1, call the `inviteUser` action path                | Rejected — the action re-checks Administrator, not just the hidden form   |
